/*
  ...
  brief:	Realizes functions  for Exerñise 18 from Task 2
...
char* Days ( unsigned short i )
{
	char* nameOfDays[5] = { "Monday", "Tuesday", "Wednesday", "Thursday", "Friday"};

	return *(nameOfDays +(i - 1));
}
...
bool ReadStruct( struct Factory *st, unsigned long i )
{
	...
	while(1)
		{
			cout<<"\n\nEnter the number of the day:  (q - quit)\n\n";
			
			control = ReadDigit((st + i) -> dayNumber);
			
			bool chack = false;
			
			for(int j = i - 1; j >= 0; j--)
				if( (st + j) -> dayNumber == (st + i) -> dayNumber )
					chack = true;
			...
			else if( control == '2' || (st + i) -> dayNumber == 0 || (st + i) -> dayNumber > 5)
			{
				cout << "\n\nYou can not enter negative, zero or more than 5 day number!\n\n";
			...
			{
				cout << "\n\nYou've already entered this day number!\n\n";
			...
			{
				cout << "\n\n" << i + 1 << " - day number, you've entered : " << (st + i) -> dayNumber << "\n\n";
		...
		while(1)
		{
			cout << "\n\nEnter the quantity of products the factory had to do during " << (st + i) -> dayNumber << " - day according to the plan:  (q - quit)\n\n";
			...
			switch(control)
			{
				...
				case '!':
					...
					continue;
				default:
					cout << "\n\nQuantity of products, the factory had to do during " << (st + i) -> dayNumber << " - day according to the plan: " << (st + i) -> plan << "\n\n";
					break;
		...
		while(1)
		{
			cout<<"\n\nEnter the quantity of products the factory has actually done during " << (st + i) -> dayNumber << " - day:  (q - quit)\n\n";
			...
			switch(control)
			{
				...
				case '!':
					...
					continue;
				default:
					cout << "\n\nQuantity of products the factory has actually done during " << (st + i) -> dayNumber << " - day:" << (st + i) -> actualyDone<<"\n\n";
					break;
...
void PrintStruct( struct Factory *st, unsigned long m )
{
	...
	cout << "\n\n     Day      Plan      Actually Done      (Actually Done/Plan)*100%\n\n";
	
	for( int i  = 0; i < m; i++ )
		cout << setw(9) << Days((st + i) -> dayNumber) << setw(11) << (st + i) -> plan << setw(14) << (st + i) -> actualyDone << setw(24) << (st + i) -> percentage << "\n\n";
}
