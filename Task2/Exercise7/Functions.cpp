/*
  ...
  brief:	Realizes functions  for Exerñise 7 from Task 2
...
struct Book * Resize( struct Book * a, int newsize, int oldsize )
{
...
bool ReadStruct( struct Book *book, unsigned long i )
{
	...
	( book + i ) -> publisher = new char[20];

	if(!ReadAtribute( ( book + i ) -> year, ( book + i ) -> autor, i, "Author", false ) )
		return false;
	if(!ReadAtribute( ( book + i ) -> year, ( book + i ) -> title, i, "Title", false ) )
		return false;
	if(!ReadAtribute( ( book + i ) -> year, ( book + i ) -> publisher, i, "Publisher", false ) )
		return false;
	if(!ReadAtribute( ( book + i ) -> year, "0", i, "Year", true ) )
		return false;
...
void PrintStruct( struct Book *st, unsigned long m )
{
	...
	cout << "\n\n    Autor                 Title                   Year                  Publisher\n\n";
	
	for( int i  = 0; i < m; i++ )
		cout << setw(9) << (st + i) -> autor << setw(11) << (st + i) -> title << setw(14) << (st + i) -> year << setw(24) << (st + i) -> publisher << "\n\n";
}
...
void CBooks( struct Book *p, unsigned long m )
{
	bool check = false;
	
	for(int i=0; i<m; i++)
		for(int j=0;j<strlen( (p+i) -> title ); j++)
		if((*((p+i)->title+j) == 'C' || *((p+i)->title+j) == 'c') && *((p+i)->title+(j+1)) == '/' && 
		   (*((p+i)->title+(j+2)) == 'C' || *((p+i)->title+(j+2)) == 'c') && *((p+i)->title+(j+3)) == '+' && *((p+i)->title+(j+4)) == '+')
			check = true;
