/*
  ...
  brief:    Defines function prototypes for Exerñise 7 from Task 2
...
bool ReadStruct( struct Book*, unsigned long ); // fill the struct's fields with some values

void PrintStruct( struct Book*, unsigned long ); // prints the fild's values of the structs

struct Book * Resize( struct Book*, int, int ); // changes  size of the massive of the structs
...
void CBooks( struct Book *p, unsigned long m ); // searches the books with "C/C++" in their titles
